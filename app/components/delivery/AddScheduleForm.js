...
function AddScheduleForm({ navigation, route }) {
  ...
  const sortOM = () => {
    ...
    let newschedule = [];
    if (orderMode.order_option) {
      //remove same date
      newschedule = JSON.parse(orderMode.preorder_option).option;
  ...
  const updateOrderMode = (newdata) => {
    apiClient.put(endpoint + orderMode.om_id, newdata).then((response) => {
      if (!response.ok) {
        ...
        console.log("updated!");
        navigation.navigate("OrderScheduleEdit");
        Toast.show({
          type: "success",
          text1: "Data Update",
  ...
  return (
    <ScrollView>
      ...
      <Box width="100%" alignItems="flex-end" mt="5" mb="20" px="5">
        <Button
