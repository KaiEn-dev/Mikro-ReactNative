...
function PreorderEdit({ route, navigation }) {
  ...
  const checkData = () => {
    let newdata = [];
    if (status == false && orderMode.order_now == 0) {
      newdata = newdata.concat(
        "At least one between ordernow/preorder modes must be active at all time!"
      );
    }
    if (status == true && orderMode.preorder_option == null) {
      newdata = newdata.concat(
        "Set up preorder schedule before activating preorder!"
      );
    }
